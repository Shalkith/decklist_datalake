...
commanders as (
    select deck_id,GROUP_CONCAT(card_name order by card_name asc SEPARATOR '[and]') as card_name from {{ref('pedh_board_commanders')}}
    GROUP BY deck_id 
),
distinct_cards as (
    select deck_id,count(distinct card_name) count from {{ref('pedh_decklists')}}
    group by 1
)
